...
* String handling
    * A lot of interop deals with strings in some form or another and handling them
      ...
      enables lazy evaluation/marshalling and encoding of native strings and managed
      strings. These allow a simple `byte[]` to store a native string and ONLY marshals to a
...
* Delegates and NativeCallbacks as Function pointers
    * Function pointers are a new feature of C# that makes for very high performance interop
      ...
      data while still supporting AOT scenarios. (NOTE: 
      `Marshal.GetFunctionPointerForDelegate()` must dynamically emit a thunk that contains
