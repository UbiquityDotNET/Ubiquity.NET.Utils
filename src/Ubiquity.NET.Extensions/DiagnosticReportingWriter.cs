...
namespace Ubiquity.NET.Extensions
{
    ...
    /// when <see cref="TextWriter.NewLine"/> is detected. When a diagnostic is reported then the internal builder is
    /// cleared so that the next line is a new diagnostic. <see cref="MsgLevel"/> is used to specify the message level
