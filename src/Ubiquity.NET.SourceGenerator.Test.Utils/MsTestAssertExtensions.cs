...
{
    // NOTE: due to bug https://github.com/dotnet/roslyn/issues/78042 this is not convertible to
    // the new 'extension' keyword. The problem is in the compiler generated lambda for the LINQ
    // expressions. Fortunately, the "old" syntax still works fine...

    /// <summary>Utility class to implement extensions for <see cref="Assert.That"/></summary>
    public static class MsTestAssertExtensions
    {
            ...
            )
        {
            ...
            foreach (var (trackingName, runSteps1) in trackedSteps1)
            {
                ...
                Assert.That.AreEqual(runSteps1, runSteps2, trackingName);
            }
        }
        ...
        public static void Cached( this Assert _, GeneratorDriverRunResult driverRunResult )
        {
            ...
            {
                Assert.Fail($"Step name {stepTrackingName ?? "<null>"} contains uncached results for second run!");
            }
        }

        /// <summary>Extension method for use with <see cref="Assert.That"/> to validate that an object is not of a banned type</summary>
        /// <param name="_">[ignored] syntactic sugar for extension method</param>
            ...
            )
        {
            ...
            if (node is not null)
            {
                ...
                Assert.IsNotInstanceOfType<Compilation>(node, msg);
                Assert.IsNotInstanceOfType<ISymbol>(node, msg);
                Assert.IsNotInstanceOfType<SyntaxNode>(node, msg);
            }
        }
        ...
        /// <summary>Extension method to validate that the output of a <see cref="IncrementalGeneratorRunStep"/> doesn't use any banned types.</summary>
        ...
        /// </remarks>
        public static void ObjectGraphContainsValidSymbols(
            this Assert _,
            IncrementalGeneratorRunStep runStep,
            string stepTrackingName
            )
